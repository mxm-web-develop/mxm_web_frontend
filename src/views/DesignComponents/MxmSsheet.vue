...
<template>
  ...
  >
    <div
      class="max-w-screen-2xl h-designbox reactive w-full overflow-hidden py-5"
    >
      ...
      <div class="flex w-full px-12 justify-between">
        <div class="mt-8 w-56 flex-grow-0 left flex flex-col border-r">
          <div
            v-for="i in modules"
            :key="i.text"
            class="py-3 font-animate-2 cursor-pointer"
            @click="() => changeActivedModule(i.text)"
            :class="
              activedComponent.name === i.text
                ? ' text-theme-red opacity-100'
                : 'opacity-50'
            "
          >
            <span
              class="w-3 h-1 bg-black"
              :class="
                activedComponent.name === i.text
                  ? ' text-theme-red text-xl transform rotate-90'
                  : ''
              "
            ></span>
            {{ i.text }}
          </div>
          <div class="font-animate-2 mt-44"><span>-</span> Export</div>
        </div>
